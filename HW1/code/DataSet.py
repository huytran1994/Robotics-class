...
class DataSet(object):
    ...
    def __init__(self, filename):
        ...
        self.states = []  # An array of (x, y) coordinates, list of sequences of states (2-tuple)
        self.observations = [] #list of sequences of observations

    def readFile(self):
        """Read in file and populate training state and output sequences."""
        states = [] #to be appended by self.states
        obs = []
        ...
        with open(self.filename, 'r') as f:
            for line in f:
                if line[0] == '#': #skip first line
                    continue
                elif line[0] == '.': #separate sequences 
                    self.states.append(states) #append sequence
                    self.observations.append(obs) #append sequence of obs
                    states = [] #reset
                    obs = [] #reset 
                else:
