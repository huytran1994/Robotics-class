...
if __name__ == '__main__':
    ...
    if (len(sys.argv) < 2 or len(sys.argv) > 3):
        print("Usage: python RunViterbi.py ../data/randomwalk.test.txt trained-model.pkl")
        sys.exit(0)
    ...
    for i in range(len(dataset.observations)):
        ...
        if len(predictedStates) != len(dataset.states[i]): #dataset.states[i]?
    ...
    print(f"Number of incorrect predictions: {totalIncorrect}")
